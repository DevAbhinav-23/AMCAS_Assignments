...
void PrintTime(const std::string &s, double seconds) {
  // PATCH(AMCAS A1): print integer microseconds instead of %f. musl's
  // float printf path (fmt_fp) faults in gem5 SE mode (writes past the
  // stack top), so gem5 runs need an integer-only print. Native timing
  // behaviour is unchanged.
  printf("%-21s%8ld us\n", (s + ":").c_str(),
         (long)(seconds * 1e6 + 0.5));
}
